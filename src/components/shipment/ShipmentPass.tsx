...
export interface ShipmentPassProps {
  ...
  onConfirm?: () => void;
  // The layoutId of the route card this pass morphed from
...
// The preserved stub shown after tear — bottom half of the pass
...
export function ShipmentPass({ route, shipment, onConfirm, morphLayoutId }: ShipmentPassProps) {
  ...
  const handleConfirm = () => {
    ...
    // After tear animation completes, signal parent
    setTimeout(() => {
      setTearPhase("torn");
  ...
  return (
    <motion.div
      // Shared layoutId with the route card — this is the morph anchor
    ...
    >
      ...
      <AnimatePresence>
        {tearPhase === "tearing" && (
          <>
            {/* Top half — slides up and out, representing the "dispatch" half going live */}
            <motion.div
              key="tear-top"
              ...
              style={{ height: "52%" }}
            />
            {/* Tear line — a bright flash at the split point */}
            <motion.div
              key="tear-line"
              ...
              style={{ top: "50%" }}
            />
            {/* Bottom half — stays, becomes the stub */}
            <motion.div
              key="tear-bottom"
              initial={{ y: 0, opacity: 1 }}
        ...
        <div className="flex items-start justify-between mb-1">
          ...
          {route.recommended ? (
            <div className="text-right mt-1 space-y-1">
              <p className="text-[10px] text-primary uppercase tracking-widest">Recommended</p>
      ...
      {/* ── CORE DATA BLOCK — offset, not centered ── */}
      <div className="grid grid-cols-2 gap-px bg-border mx-0 my-0 border-b border-border">
      ...
      <Separator className="opacity-20 mx-0" />

      {/* ── LOWER METADATA — left-aligned, not centered ── */}
      ...
      {/* ── TEAR LINE (dashed) ── */}
      <div className="border-t border-dashed border-border/50 mx-0 pt-5 pb-5 px-6">
        <ShipmentCode code={shipment.shipmentCode} />
      </div>
